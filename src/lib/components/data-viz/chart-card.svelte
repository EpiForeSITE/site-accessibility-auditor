<script lang="ts">
	...
	let auditWarningCount = $derived(
		Number(!chart.hasAccessibleName) +
			Number(!chart.captionText && !chart.hasTableFallback) +
			Number(!chart.supportsKeyboard) +
			Number(chart.colorChannels.length >= 3 && chart.legendItems.length === 0)
	);
</script>
...
>
	...
	<div class="min-w-0 flex-1">
		...
		{#if chart.legendItems.length > 0 || chart.seriesLabels.length > 0}
			<div class="mt-1.5 text-[10px] text-[var(--panel-text-subtle)]">
				{chart.legendItems.length > 0
					? `Legend: ${chart.legendItems.slice(0, 3).join(', ')}`
					: `Series: ${chart.seriesLabels.slice(0, 3).join(', ')}`}
			</div>
		{/if}
	</div>
	...
	<div
		class="mt-0.5 shrink-0 text-right"
		title={auditWarningCount > 0
			? `${auditWarningCount} audit warnings`
			: 'No structural audit warnings'}
	>
		{#if auditWarningCount === 0}
			<svg
				class="h-4 w-4"
				style="color: var(--panel-success-text);"
				fill="none"
				stroke="currentColor"
				viewBox="0 0 24 24"
			>
